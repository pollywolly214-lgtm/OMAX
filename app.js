...
let estHours = parseInt(localStorage.getItem(LS_EST)) || 1500; // default
...
function updateTaskHours(taskName, val) {
  let t = tasks.find(t => t.name === taskName);
  if (t && !isNaN(parseInt(val))) {
    t.lastDone = parseInt(val);
...
function render() {
  ...
  renderDashboard();
  renderCalendar();
  document.getElementById("lastNozzle").value = tasks.find(t => t.name === "Nozzle Change").lastDone;
  document.getElementById("lastOil").value = tasks.find(t => t.name === "Pump Oil").lastDone;
}
