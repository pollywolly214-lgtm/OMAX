...
function parseNumericLineNumbers(line) {
  const text = String(line || '').trim();
  if (!text) return [];
  if (!/^[\s,;:+\-\d.eE]+$/.test(text)) return [];
  return [...text.matchAll(/[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?/g)]
    .map(match => Number.parseFloat(match[0]))
    .filter(Number.isFinite);
}

function chooseNumericColumns(lines) {
  const rows = lines.map(parseNumericLineNumbers);
  const maxLen = rows.reduce((acc, row) => Math.max(acc, row.length), 0);
  if (maxLen < 2) return null;

  let best = null;
  const limit = Math.min(8, maxLen);
  for (let xi = 0; xi < limit - 1; xi += 1) {
    for (let yi = xi + 1; yi < limit; yi += 1) {
      const points = [];
      rows.forEach(row => {
        if (row.length <= yi) return;
        points.push({ x: row[xi], y: row[yi] });
      });

      const score = scorePointStream(points);
      if (!best || score > best.score) {
        best = { xIndex: xi, yIndex: yi, score };
      }
    }
  }
...
function scorePointStream(points) {
  ...
  const thinness = Math.min(spanX, spanY) / span;
  ...
  for (let i = 1; i < points.length; i += 1) {
    ...
    if (jump > span * 20) hugeJumps += 1;
  }

  const continuity = connected / Math.max(1, points.length - 1);
  const jumpPenalty = hugeJumps / Math.max(1, points.length - 1);
  return (continuity * 100) + (thinness * 25) + Math.log10(span + 1) - (jumpPenalty * 60);
}
