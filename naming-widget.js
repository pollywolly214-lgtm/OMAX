...
function renderCoordinateCloudToSvgDataUrl(text) {
  ...
  const numericPair = inferNumericPair(lines);
  ...
  lines.forEach(line => {
    ...
    const points = extractLinePoints(line, numericPair);
...
function inferNumericPair(lines) {
  const sample = lines.slice(0, 400)
    .map(line => String(line || '').trim())
    .filter(line => line && !/[A-Za-z]/.test(line))
    .map(line => [...line.matchAll(/[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?/g)].map(match => Number.parseFloat(match[0])).filter(Number.isFinite))
    .filter(numbers => numbers.length >= 2);

  if (sample.length < 6) return null;

  const maxCols = Math.min(8, sample.reduce((m, row) => Math.max(m, row.length), 0));
  let best = null;

  for (let xIndex = 0; xIndex < maxCols - 1; xIndex += 1) {
    for (let yIndex = xIndex + 1; yIndex < maxCols; yIndex += 1) {
      const points = sample
        .filter(row => row.length > yIndex)
        .map(row => ({ x: row[xIndex], y: row[yIndex] }))
        .filter(point => Number.isFinite(point.x) && Number.isFinite(point.y));

      if (points.length < 6) continue;
      const score = scoreNumericPair(points);
      if (!best || score > best.score) best = { xIndex, yIndex, score };
    }
  }
...
function scoreNumericPair(points) {
  ...
  const spanScore = Math.log10(Math.max(1, spanX)) + Math.log10(Math.max(1, spanY));

  let connected = 0;
  let hugeJump = 0;
  for (let i = 1; i < points.length; i += 1) {
    const jump = Math.hypot(points[i].x - points[i - 1].x, points[i].y - points[i - 1].y);
    if (jump > 0 && jump < 50000) connected += 1;
    if (jump > 50000) hugeJump += 1;
  }

  return (connected * 2) + spanScore - (hugeJump * 1.5);
}

function extractLinePoints(line, numericPair = null) {
  ...
  if (numericPair && numbers.length > numericPair.yIndex) {
    return [{ x: numbers[numericPair.xIndex], y: numbers[numericPair.yIndex] }];
  }
