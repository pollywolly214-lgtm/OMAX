...
async function preparePreviewData(file) {
  ...
  const cadPreviewSvg = renderCadToSvgDataUrl(content);
  const coordCandidate = buildCoordinatePreviewCandidate(content);
  const binaryCandidate = buildBinaryPreviewCandidate(buffer);
  const bestFallback = pickBestPreviewCandidate([coordCandidate, binaryCandidate]);

  const previewSvg = cadPreviewSvg || bestFallback?.svg || '';
...
function buildCoordinatePreviewCandidate(text) {
  ...
  const svg = buildPathSvgDataUrl(path, bounds);
  return { svg, score: previewCandidateScore(bounds, path.length, pointsDrawn.length) };
}

function renderCoordinateCloudToSvgDataUrl(text) {
  return buildCoordinatePreviewCandidate(text)?.svg || '';
}
...
function buildBinaryPreviewCandidate(buffer) {
...
function renderBinaryFloatPairsToSvgDataUrl(buffer) {
  return buildBinaryPreviewCandidate(buffer)?.svg || '';
}

function pickBestPreviewCandidate(candidates) {
...
function previewCandidateScore(bounds, pathTokenCount, drawnPointCount) {
  const width = Math.max(1, bounds.maxX - bounds.minX);
  const height = Math.max(1, bounds.maxY - bounds.minY);
  const aspect = Math.max(width / height, height / width);
  const areaSignal = Math.log10((width * height) + 1);
  const detailSignal = Math.min(300, pathTokenCount) / 10;
  const drawSignal = Math.min(200, drawnPointCount) / 8;
  const aspectPenalty = Math.max(0, aspect - 15) * 1.5;
  return areaSignal + detailSignal + drawSignal - aspectPenalty;
}
