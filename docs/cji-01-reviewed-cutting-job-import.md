...
## CJI-01F blank-material review and compact preview

A genuinely blank workbook material now resolves only to one unambiguous live `A36 steel` definition. Preview labels this case **A36 steel (assumed)** and classifies it separately from exact, approved-alias, and unresolved matches, while the saved job uses the canonical `A36 steel` name. The untouched blank cell remains in `importProvenance.material`; structured provenance records the assumption flag, canonical name, and reason, and the job note explains the assumption. Missing or ambiguous A36 settings block the row, nonblank values such as RC50 never enter the assumption path, and final validation rejects a changed A36 calculation before backup or mutation.

The importer dialog now has a compact, wrapping control/summary section and a separately bounded two-axis table scroller. Rows use content height and compact padding, numeric columns retain practical minimum widths, long reasons/warnings/notes receive readable widths, and the header remains sticky. The summary reports ready/unresolved, assumed/exact/alias, RC50, and cost completeness counts without hiding row-level safety details. The 67-row production workbook was not present in the repository or `/workspace`, so no real-workbook counts are claimed and no import or Firebase write was performed.
