...
The create-only file document at `workspaces/{workspaceId}/cuttingJobs/{jobId}/files/{fileId}` contains exactly: `schemaVersion`, `workspaceId`, `jobId`, `fileId`, `originalName`, `safeFileName`, `extension`, `contentType`, `sizeBytes`, `storagePath`, `sha256`, `status`, `createdBy`, and `createdAtISO`. The compatibility document at `workspaces/{workspaceId}/cuttingJobs/{jobId}` contains only schema/workspace/job identity, the authoritative state path, and creation time. Neither is a replacement for the full-state job.
...
`storage.rules` is a **blocked production proposal**, not deployed here. It defaults all unrelated paths to deny, allows no list/update, scopes create/get/delete to the immutable path, validates authentication/membership/roles/path/custom metadata/type/50-MiB limit, and does not restore CFR-03. Delete exists solely for failed-upload compensation and requires the uploader or owner/admin.
