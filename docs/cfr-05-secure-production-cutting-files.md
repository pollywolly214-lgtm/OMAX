...
**Implementation status:** The client workflow is implemented, and the reviewed Firestore and Storage rules in this repository mirror rules deployed manually to WJ TRACKER-V2. This repository repair did not deploy rules, contact Firebase, upload/download a live attachment, exercise browser preview, perform cross-device testing, mutate caches, or migrate production data.
...
Before any live attachment test, operators must confirm the deployed rule versions, membership protection, individual metadata access, Storage Rules cross-service Firestore access, and a valid administratively created membership. This repair did not inspect authoritative production job counts. A live test must use an existing legitimate authoritative job and must never create a synthetic production job merely to hold files. Browser attachment and cross-device behavior remain unverified by this repair.
