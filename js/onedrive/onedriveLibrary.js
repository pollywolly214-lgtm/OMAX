(function(){
  ...
  async function resolveSharedFile(shareUrl){
    const token = encodeSharingUrlToToken(shareUrl);
    if (!token) throw new Error("Enter a valid OneDrive file share link.");
    const endpoints = [
      `https://graph.microsoft.com/v1.0/shares/${encodeURIComponent(token)}/driveItem`,
      `https://api.onedrive.com/v1.0/shares/${encodeURIComponent(token)}/driveItem`
    ];
    ...
    for (const url of endpoints){
      try {
        ...
        if (!item?.file) throw new Error("That link is a folder. Paste a file link to attach.");
        const name = String(item?.name || "");
        const ext = extOf(name);
        if (![".dxf", ".ord", ".omx"].includes(ext)){
          throw new Error("Only DXF/ORD/OMX files can be attached from OneDrive.");
        }
        const driveId = String(item?.parentReference?.driveId || item?.remoteItem?.parentReference?.driveId || "");
        if (!driveId || !item?.id){
          throw new Error("Unable to read OneDrive file reference.");
        }
        return {
          id: String(item.id),
          ...
          webUrl: String(item?.webUrl || shareUrl || ""),
          eTag: String(item?.eTag || ""),
          lastModifiedDateTime: String(item?.lastModifiedDateTime || ""),
          size: Number(item?.size || 0)
        };
    ...
    throw lastError || new Error("Unable to resolve OneDrive file link.");
  }
  ...
  window.oneDriveLibrary = { extOf, encodeSharingUrlToToken, crawlSharedFolder, resolveSharedFile, readCache, writeCache };
})();
