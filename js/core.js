...
const cuttingJobRepairEnv={state:getCuttingJobImporterState,categories:()=>window.jobFolders||[],setCategories:value=>{setJobFolders(value);jobFolders=window.jobFolders;},revalidateBaseline:async()=>{if(!FB.docRef||!FB.user)return false;const snap=await FB.docRef.get();if(!snap.exists)return false;return Number((snap.data()||{}).syncMeta?.rev||0)===Number((window.__lastLoadedCloudState||{}).syncMeta?.rev||0);},backup:cuttingJobRepairBackup,saveCloudNow:()=>saveCloudNow()};
window.auditCuttingJobHistoryRepair=()=>window.CuttingJobRepair.audit(getCuttingJobImporterState(),window.jobFolders||[]);
window.runCuttingJobHistoryRepair=()=>window.CuttingJobRepair.repair(cuttingJobRepairEnv);
