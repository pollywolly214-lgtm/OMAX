...
/* ====================== FIREBASE =========================== */
let FB = { app:null, auth:null, db:null, user:null, docRef:null, workspaceRef:null, workspaceDoc:null, ready:false };
...
async function initFirebase(){
  ...
  FB.auth.onAuthStateChanged(async (user)=>{
    ...
    if (user){
      ...
      // Store workspace data under workspaces/<id>/app/state to mirror the legacy layout.
...
async function loadFromCloud(){
  ...
  try{
    if (FB.workspaceDoc){
      try {
        const hasFirebase = typeof firebase !== "undefined" && firebase.firestore && firebase.firestore.FieldValue;
        const metaUpdate = {
          workspaceId: WORKSPACE_ID,
          lastTouchedAt: (hasFirebase && typeof firebase.firestore.FieldValue.serverTimestamp === "function")
            ? firebase.firestore.FieldValue.serverTimestamp()
            : new Date().toISOString()
        };
        await FB.workspaceDoc.set(metaUpdate, { merge:true });
      } catch (metaErr) {
    ...
    if (snap.exists){
      ...
      if (FB.workspaceDoc){
        try {
          ...
          const metaSeed = {
            workspaceId: WORKSPACE_ID,
            createdAt: (hasFirebase && typeof firebase.firestore.FieldValue.serverTimestamp === "function")
              ? firebase.firestore.FieldValue.serverTimestamp()
              : new Date().toISOString()
          };
          await FB.workspaceDoc.set(metaSeed, { merge:true });
        } catch (workspaceErr) {
          console.warn("Failed to seed workspace metadata", workspaceErr);
        }
      }
    }
    if (window.DEBUG_MODE){
