...
function renderJobs(){
  ...
  const oneDriveOpenLink = document.querySelector("#jobOneDriveOpenLink");
  ...
  oneDriveSaveBtn?.addEventListener("click", ()=>{
    ...
    const normalizedLink = window.oneDriveLibrary?.normalizeSharedUrl?.(link) || link;
    const next = updateSharedConfig({
      enabled: !!oneDriveEnabledInput?.checked,
      sharedFolderUrl: normalizedLink,
  ...
  const setOneDriveExplorerLink = (url)=>{
    const safeUrl = String(url || "").trim();
    if (oneDriveEmbeddedFrame){ oneDriveEmbeddedFrame.src = safeUrl || "about:blank"; }
    if (oneDriveOpenLink){
      oneDriveOpenLink.href = safeUrl || "#";
      oneDriveOpenLink.style.pointerEvents = safeUrl ? "auto" : "none";
      oneDriveOpenLink.style.opacity = safeUrl ? "1" : ".6";
    }
  };
  ...
  oneDriveLibraryAddToJobBtn?.addEventListener("click", async ()=>{
    ...
    setOneDriveExplorerLink(cfg.sharedFolderUrl);
