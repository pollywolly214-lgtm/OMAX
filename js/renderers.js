...
function computeCostModel(){
  ...
  const getTaskPartsCost = (task)=>{
    const parts = Array.isArray(task?.parts) ? task.parts : [];
    let total = 0;
    parts.forEach(part => {
      if (!part) return;
      const raw = Number(part.price);
  ...
  const buildCalendarMaintenanceHistory = ()=>{
    ...
    const addEntry = (task, dateKey)=>{
      ...
      const partsTotal = getTaskPartsCost(task);
      const fallbackPrice = Number.isFinite(priceRaw) && priceRaw > 0
        ? priceRaw
        : (Number.isFinite(partsTotal) && partsTotal > 0 ? partsTotal : 0);
  ...
  const intervalTaskRowsRaw = intervalTasks.map((task, index) => {
    ...
    const unitPrice = Number.isFinite(price) && price > 0
      ...
      : (Number.isFinite(calendarPrice) && calendarPrice > 0
        ? calendarPrice
        : (Number.isFinite(partsTotal) && partsTotal > 0 ? partsTotal : null));
