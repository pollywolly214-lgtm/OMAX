...
function renderSettings(){
  ...
  if (!document.getElementById("settingsExplorerCSS")){
    ...
    st.textContent = `
      ...
      #explorer .task-children.dragover{outline:2px dashed #0a63c2;outline-offset:3px;background:rgba(10,99,194,.04)}
  ...
  tree?.addEventListener('dragend',()=>{
    ...
    tree.querySelectorAll('[data-task-children].dragover').forEach(el=>el.classList.remove('dragover'));
  ...
  tree?.addEventListener('drop',(e)=>{
    ...
    if (kind === 'task' && id){
      if (onSum){
        const taskNode = onSum.closest('details.task');
        const targetId = taskNode?.getAttribute('data-task-id');
        if (targetId){
          if (String(targetId) === String(id)) return;
          const rect = onSum.getBoundingClientRect();
          const midpoint = rect.top + rect.height * 0.4;
          const cursorY = Number.isFinite(e.clientY) ? e.clientY : midpoint + 1;
          const shouldNest = cursorY > midpoint;
          if (shouldNest){
            if (typeof moveNodeSafely === 'function' && moveNodeSafely('task', id, { intoTask: targetId })){
              persist();
              renderSettings();
            }
          }else{
            if (typeof moveNodeSafely === 'function' && moveNodeSafely('task', id, { beforeTask: { id: targetId } })){
              persist();
              renderSettings();
            }
          }
      ...
      if (!dzTask && taskWrap){
        const parentId = taskWrap.getAttribute('data-task-children');
        if (parentId && typeof moveNodeSafely === 'function' && moveNodeSafely('task', id, { intoTask: parentId })){
          persist();
          renderSettings();
        }
        return;
