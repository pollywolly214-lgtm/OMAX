...
function computeCostModel(){
  ...
  const buildCalendarMaintenanceHistory = ()=>{
    ...
    const addEntry = (task, dateKey)=>{
      ...
      const fallbackPrice = Number.isFinite(priceRaw) && priceRaw > 0 ? priceRaw : 0;
  ...
  const historyCostByTask = new Map();
  calendarMaintenanceHistory.forEach(entry => {
    if (!entry || !(entry.date instanceof Date)) return;
    ...
    tasks.forEach(task => {
      const key = task?.originalId != null ? String(task.originalId) : (task?.id != null ? String(task.id) : null);
      if (!key) return;
      const stats = historyCostByTask.get(key) || { total: 0, count: 0, latestCost: null, latestTime: null };
      stats.total += cost;
      stats.count += 1;
      if (stats.latestTime == null || time > stats.latestTime){
        stats.latestTime = time;
        stats.latestCost = cost;
      }
      historyCostByTask.set(key, stats);
    });
  });

  const getForecastUnitPrice = (task)=>{
    const priceRaw = Number(task?.price);
    if (Number.isFinite(priceRaw) && priceRaw > 0) return priceRaw;
    const key = resolveTaskTemplateKey(task);
    const stats = key ? historyCostByTask.get(key) : null;
  ...
  const intervalCostPerHour = intervalTasks.reduce((sum, task)=>{
    const price = getForecastUnitPrice(task);
