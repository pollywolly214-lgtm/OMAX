...
function refreshDashboardWidgets(){
  ...
  const runRefresh = ()=>{
    if (pendingDashboardRefresh == null) return;
  ...
  // Use a timeout as a fallback so the widgets still refresh if rAF is throttled
  // (e.g., when the dashboard is backgrounded or inside a modal overlay).
  setTimeout(runRefresh, 30);
}
