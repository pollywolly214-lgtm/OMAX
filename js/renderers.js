...
function computeCostModel(){
  ...
  const hasOccurrenceDates = (task)=>{
    if (!task) return false;
    const notes = (typeof normalizeOccurrenceNotes === "function")
      ? normalizeOccurrenceNotes(task)
      : (task.occurrenceNotes || {});
    if (notes && typeof notes === "object" && Object.keys(notes).some(hasDateKey)){
      return true;
    }
    const hours = (typeof normalizeOccurrenceHours === "function")
      ? normalizeOccurrenceHours(task)
      : (task.occurrenceHours || {});
    if (hours && typeof hours === "object" && Object.keys(hours).some(hasDateKey)){
      return true;
    }
    return false;
  };
  ...
  const taskLookupById = new Map();
  intervalTasksAll.forEach(task => {
    if (task && task.id != null){
      taskLookupById.set(String(task.id), task);
    }
  });
  asReqTasksAll.forEach(task => {
    if (task && task.id != null){
      taskLookupById.set(String(task.id), task);
    }
  });
  ...
  const computeTaskPartsCost = (task)=>{
    if (!task) return 0;
    const sumParts = (parts)=> Array.isArray(parts)
      ...
      : 0;
    const directTotal = sumParts(task.parts);
    if (directTotal > 0) return directTotal;
    const templateId = task.templateId != null ? String(task.templateId) : null;
    if (!templateId) return 0;
    const template = taskLookupById.get(templateId);
    if (!template || template === task) return 0;
    return sumParts(template.parts);
  };
