...
function computeCostModel(){
  ...
  const buildMaintenanceRows = (taskList, modeLabel)=>{
    ...
    (Array.isArray(taskList) ? taskList : []).forEach((task, index) => {
      ...
      const partsCostYear = stats.completedYear * unitCostSafe;
      const partsCostTwoMonth = completedTwoMonth * unitCostSafe;
      const totalCostYear = timeCostTotalYear + partsCostYear;
      const totalCostTwoMonth = timeCostTotalTwoMonth + partsCostTwoMonth;
      rows.push({
        ...
        partsCostYearLabel: partsCostYear > 0
        ...
        partsCostTwoMonthLabel: partsCostTwoMonth > 0
