...
function computeCostModel(){
  ...
  const resolveForecastUnitPrice = (task)=>{
    if (!task) return null;
    const rawPrice = Number(task?.price);
    if (Number.isFinite(rawPrice) && rawPrice > 0) return rawPrice;
    const occurrenceCost = resolveOccurrenceCostEstimate(task);
    if (Number.isFinite(occurrenceCost) && occurrenceCost > 0) return occurrenceCost;
    return null;
  };
  ...
  const intervalCostPerHour = intervalTasks.reduce((sum, task)=>{
    const price = resolveForecastUnitPrice(task);
