...
function renderCosts(){
  ...
  const pendingWeeklyScrollState = (typeof window !== "undefined" && window.__costWeeklyScrollState && typeof window.__costWeeklyScrollState === "object")
    ? { ...window.__costWeeklyScrollState }
    : null;
  if (pendingWeeklyScrollState && typeof window !== "undefined"){
    window.__costWeeklyScrollState = null;
    const restoreWeeklyScroll = ()=>{
      const weeklyPanel = content.querySelector("[data-cost-weekly-reports]");
      if (weeklyPanel instanceof HTMLElement){
        weeklyPanel.scrollTop = Number.isFinite(Number(pendingWeeklyScrollState.panelScrollTop))
          ? Number(pendingWeeklyScrollState.panelScrollTop)
          : 0;
      }
      if (Number.isFinite(Number(pendingWeeklyScrollState.pageScrollY))){
        window.scrollTo(0, Number(pendingWeeklyScrollState.pageScrollY));
      }
    };
    if (typeof window.requestAnimationFrame === "function"){
      window.requestAnimationFrame(restoreWeeklyScroll);
    } else {
      setTimeout(restoreWeeklyScroll, 0);
    }
  }
  ...
  function setupMaintenanceDataCenterActions(options = {}){
    ...
    const pulseRow = (row)=>{
      ...
      row.style.backgroundColor = "rgba(52, 120, 246, 0.28)";
      setTimeout(()=>{
    ...
    const focusDataCenterRow = (payload = {})=>{
      if (!(modal instanceof HTMLElement)) return false;
  ...
  function setupWeeklyReportWindow(currentModel){
    const panel = content.querySelector("[data-cost-weekly-reports]");
    if (!panel) return;

    const reports = Array.isArray(currentModel?.weeklyReports) ? currentModel.weeklyReports : [];
    const selected = (typeof window !== "undefined" && typeof window.weeklyCostReportSelected === "string")
      ? window.weeklyCostReportSelected
      : "";
    const fallback = reports.length ? String(reports[0].weekStartISO || "") : "";
    const normalized = reports.some(r => String(r?.weekStartISO || "") === selected) ? selected : fallback;
    if (typeof window !== "undefined") window.weeklyCostReportSelected = normalized;

    const selectEl = panel.querySelector("[data-cost-weekly-select]");
    if (selectEl instanceof HTMLSelectElement){
      selectEl.addEventListener("change", (event)=>{
        const val = event && event.target instanceof HTMLSelectElement ? event.target.value : selectEl.value;
        if (typeof window !== "undefined"){
          window.weeklyCostReportSelected = String(val || "");
          const pageScrollY = Number.isFinite(Number(window.scrollY)) ? Number(window.scrollY) : 0;
          const panelScrollTop = panel instanceof HTMLElement ? panel.scrollTop : 0;
          window.__costWeeklyScrollState = { pageScrollY, panelScrollTop };
        }
        renderCosts();
      });
    }

    const exportBtn = panel.querySelector("[data-cost-weekly-export]");
    if (exportBtn instanceof HTMLElement){
      exportBtn.addEventListener("click", ()=>{
        ...
        const escHtml = (value)=> String(value == null ? "" : value)
          ...
          .replace(/>/g, "&gt;")
          .replace(/"/g, "&quot;");
        const weekTitle = report.weekLabel || report.weekStartISO || "Weekly report";
        ...
        const maintRows = (Array.isArray(report.maintenanceItems) ? report.maintenanceItems : []).map(item => `
          <tr>
            <td>${escHtml(item?.name || "Maintenance task")}</td>
            <td>${escHtml(item?.taskLabel || "Maintenance")}</td>
        ...
        const url = URL.createObjectURL(blob);
        link.href = url;
        link.download = `weekly-cost-report-${report.weekStartISO || "week"}.xls`;
    ...
    const buildWorkbookFromTable = ({ title, subtitle, headerRows, bodyRows, footerRows = [] })=> `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
      ...
      <table>
        <thead>${(Array.isArray(headerRows) ? headerRows : []).map(row => `<tr>${row.map(col => `<th>${escWorkbookHtml(col)}</th>`).join("")}</tr>`).join("")}</thead>
    ...
    const shiftMonthsBack = (anchorDate, monthsBack)=>{
      const base = anchorDate instanceof Date ? anchorDate : new Date();
      const m = Number(monthsBack);
      if (!Number.isFinite(m) || m <= 0) return null;
      const dayOfMonth = base.getDate();
      const startOfTargetMonth = new Date(base.getFullYear(), base.getMonth() - m, 1);
      const lastDay = new Date(startOfTargetMonth.getFullYear(), startOfTargetMonth.getMonth() + 1, 0).getDate();
      const boundedDay = Math.min(dayOfMonth, lastDay);
      const shifted = new Date(startOfTargetMonth);
      shifted.setDate(boundedDay);
      shifted.setHours(0, 0, 0, 0);
      return shifted;
    };
    const getRangeWindow = (months)=>{
      const today = new Date();
      today.setHours(23, 59, 59, 999);
      const start = months === "all" ? null : shiftMonthsBack(today, months);
      return { start, end: today };
    };
    const formatDateRangeLabel = (start, end)=> {
      const endIso = end instanceof Date && !Number.isNaN(end.getTime()) ? end.toISOString().slice(0, 10) : "";
      if (!start) return endIso ? `Range: All time through ${endIso}` : "Range: All time";
      const startIso = start instanceof Date && !Number.isNaN(start.getTime()) ? start.toISOString().slice(0, 10) : "";
      return startIso && endIso ? `Range: ${startIso} to ${endIso}` : "Range: Date window unavailable";
    };
    const buildRangeRows = (months)=>{
      const { start, end } = getRangeWindow(months);
      const rows = [];
    ...
    if (receiptOpenBtn instanceof HTMLElement && modal instanceof HTMLElement){
      ...
      if (exportWeekBtn instanceof HTMLElement){
        exportWeekBtn.addEventListener("click", ()=>{
          ...
          const weekRange = entry.startISO && entry.endISO
            ? `Week: ${entry.startISO} to ${entry.endISO}`
            : `Week: ${entry.key || "Selected week"}`;
...
function computeCostModel(){
  ...
  const startOfWeekMonday = (value)=>{
    let dt = null;
    if (value instanceof Date) dt = new Date(value);
    else if (typeof parseDateLocal === "function") dt = parseDateLocal(value) || new Date(value);
  ...
  const resolveCuttingJobHours = (job, eff = null)=>{
    ...
    const positive = candidates.find(val => Number.isFinite(val) && val > 0);
    const fallback = candidates.find(val => Number.isFinite(val) && val >= 0);
    return Number.isFinite(positive) ? positive : (Number.isFinite(fallback) ? fallback : 0);
  };

  const resolveCuttingJobMaterialCost = (job)=>{
  ...
  const resolveCuttingJobNetTotal = (job, eff = null)=>{
    const hours = resolveCuttingJobHours(job, eff);
    ...
    const cutCostRateRaw = Number(job?.costRate ?? eff?.costRate);
    const cutCostRate = Number.isFinite(cutCostRateRaw) && cutCostRateRaw >= 0 ? cutCostRateRaw : JOB_BASE_COST_PER_HOUR;
    const materialCost = resolveCuttingJobMaterialCost(job);
    const billedAmount = chargeRate * Math.max(0, hours);
    const cutCost = Math.max(0, hours) * cutCostRate;
    return {
      hours,
      chargeRate,
      cutCostRate,
      materialCost,
      total: billedAmount - materialCost - cutCost
    };
  };
  ...
  if (jobSeriesSorted.length){
    jobSeriesSorted.forEach((pt, idx)=>{
      ...
      const pointValue = Number(pt.rawValue) || 0;
  ...
  const rootFolderId = typeof JOB_ROOT_FOLDER_ID === "string" ? JOB_ROOT_FOLDER_ID : "jobs_root";
  folderNameById.set(rootFolderId, "All Jobs");
  (Array.isArray(jobFolders) ? jobFolders : []).forEach(folder => {
    if (!folder || folder.id == null) return;
    const id = String(folder.id);
    const name = typeof folder.name === "string" && folder.name.trim() ? folder.name.trim() : "Uncategorized";
    folderNameById.set(id, name);
  });
  const resolveCategoryName = (catId)=>{
    const key = catId != null ? String(catId) : rootFolderId;
    return folderNameById.get(key) || "Uncategorized";
  };
  ...
  const completedCutsForWeekly = (Array.isArray(completedCuttingJobs) ? completedCuttingJobs : [])
    .map(job => {
      ...
      const jobNet = resolveCuttingJobNetTotal(job, eff);
      const cutCost = Number(jobNet.total) || 0;
  ...
  completedCutsForWeekly.forEach((cut, idx)=>{
    const weekStart = startOfWeekMonday(cut.date);
    const bucket = ensureWeek(weekStart);
    if (!bucket) return;
    bucket.cutItems.push({
      id: `c_${cut.id || idx}`,
      dateISO: cut.dateISO,
      name: cut.name,
      ...
      cost: cut.cost,
      hours: cut.hours,
      costLabel: formatterCurrency(cut.cost, { showPlus: true, decimals: Math.abs(cut.cost) < 1000 ? 2 : 0 }),
      hoursLabel: formatHours(cut.hours)
    });
    bucket.totalCutCost += cut.cost;
    if (!bucket.cutByCategory[cut.category]) bucket.cutByCategory[cut.category] = { count: 0, cost: 0, hours: 0 };
    bucket.cutByCategory[cut.category].count += 1;
  ...
  const weeklyReports = Array.from(weeklyMap.values())
    .sort((a,b)=> String(b.weekStartISO).localeCompare(String(a.weekStartISO)))
    .map(report => ({
      ...report,
  ...
  const jobCategoryAnalytics = (()=>{
    ...
    try {
      ...
      const analyzeJob = (job, type)=>{
        ...
        const actualHours = resolveCuttingJobHours(job, eff);
        ...
        const materialCost = resolveCuttingJobMaterialCost(job);
        const laborCost = Math.max(0, durationHours) * (Number.isFinite(Number(job?.costRate)) ? Number(job.costRate) : JOB_BASE_COST_PER_HOUR);
        const machineCost = machineFromJob(job);
        const overheadCost = overheadFromJob(job);
        const jobNet = resolveCuttingJobNetTotal(job, eff);
        const totalCost = Number(jobNet.total) || 0;
  ...
  const chartNote = "Maintenance line shows negative loss values (blue), and cutting jobs line shows per-job net gain/loss (green when profitable, negative if jobs become unprofitable). Hover or click any point for details, then click to jump to matching Data Center rows.";
  const chartInfo = "Maintenance trend is plotted as negative cost impact, while cutting jobs are plotted as each completed job’s net result so profitability changes are visible immediately. Point hover/click reveals context and can jump to linked Data Center rows.";
  ...
  const maintenanceAverageValue = maintenanceSeries.length
    ? (maintenanceSeries.reduce((sum, item) => sum + (Number(item?.value) || 0), 0) / maintenanceSeries.length)
    : 0;
...
function drawWeeklyReportChart(canvas, report){
  if (!canvas) return;
  ...
  if (!report){
    ...
    ctx.fillText("No weekly report selected.", W/2, H/2);
    return;
  }
...
function drawCostChart(canvas, model, show){
  ...
  active.forEach(series => {
    ...
    points.forEach(pt => {
      ...
      ctx.fill();
      const valueLabel = formatMoney(Number(pt.value));
      const dateLabel = (pt.date instanceof Date && !Number.isNaN(pt.date.getTime()))
        ? pt.date.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })
        : "the latest update";
      let detail = (typeof pt.detail === "string" && pt.detail.trim()) ? pt.detail.trim() : "";
      if (!detail){
