...
function renderJobs(){
  ...
  const oneDriveFileLinkInput = document.querySelector("#jobOneDriveFileLink");
  const oneDriveAttachLinkBtn = document.querySelector("#jobOneDriveAttachLinkBtn");
  ...
  const closeOneDriveExplorer = ()=>{
    ...
    if (oneDriveFileLinkInput){ oneDriveFileLinkInput.value = ""; }
    document.body.classList.remove("modal-open");
  };

  const attachPickedOneDriveFile = (picked)=>{
    if (!picked) return;
  ...
  oneDriveExplorerList?.addEventListener("click", (event)=>{
    ...
    if (selectFile){
      ...
      if (!picked) return;
      attachPickedOneDriveFile(picked);
    }
  });

  oneDriveExplorerSearch?.addEventListener("input", ()=>{ explorerState.query = String(oneDriveExplorerSearch.value || ""); renderExplorer(); });
  oneDriveAttachLinkBtn?.addEventListener("click", async ()=>{
    const link = String(oneDriveFileLinkInput?.value || "").trim();
    if (!link){
      toast("Paste a OneDrive file share link first.");
      return;
    }
    if (!window.oneDriveLibrary?.resolveSharedFile){
      toast("OneDrive link resolver is not available.");
      return;
    }
    try {
      const picked = await window.oneDriveLibrary.resolveSharedFile(link);
      attachPickedOneDriveFile(picked);
    } catch (err){
      toast(err?.message || "Unable to attach file from link.");
    }
  });
