...
function removeCalendarTaskOccurrences(meta, dateISO, scope = "single"){
  ...
  const getDayTime = (value)=>{
    const normalized = normalizeDateKey(value);
    if (!normalized) return null;
    const day = toDayStart(normalized);
    if (!(day instanceof Date) || Number.isNaN(day.getTime())) return null;
    return day.getTime();
  };

  const matchesScope = (value)=>{
    const compareTime = getDayTime(value);
    if (compareTime == null) return false;

    if (normalizedScope === "all") return true;
    if (normalizedScope === "future") return targetTime != null ? compareTime >= targetTime : false;
    return targetTime != null ? compareTime === targetTime : false;
  };
