...
function viewJobs(){
  ...
  const buildCloudFileMarkup = jobId => {
    ...
    if (selected){
      ...
      const download = `<button type="button" class="link" data-cfr05-download="${esc(selected.fileId)}" data-cfr05-job-id="${esc(jobKey)}">Download</button>`;
      if (preview?.status === "ready"){
        previewMarkup = `<button type="button" class="job-cloud-preview-button" data-cfr05-enlarge-preview="${esc(selected.fileId)}" data-cfr05-job-id="${esc(jobKey)}" data-cfr05-sha256="${esc(selected.sha256)}" data-cfr05-file-name="${esc(selected.originalName)}" aria-label="Enlarge ${esc(selected.originalName)}"><span class="job-cloud-preview-frame"><img class="job-cloud-preview-image" src="${esc(preview.previewData)}" alt="Preview of ${esc(selected.originalName)}"></span></button><div class="job-file-preview-name">${esc(selected.originalName)}</div><div class="small muted">${esc(detail)}</div>${download}`;
      } else if (preview?.status === "unavailable" || preview?.status === "error"){
        previewMarkup = `<div class="job-cloud-preview-unavailable">DXF browser preview unavailable</div><div class="job-file-preview-name">${esc(selected.originalName)}</div><div class="small muted">${esc(detail)}</div>${download}`;
      } else {
