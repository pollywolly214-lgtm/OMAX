(function(){
  ...
  function renderOmaxToolpathToSvgDataUrl(content, ext) {
    ...
    return buildPathSvgDataUrl(path, boundsFromPoints(focusPoints));
  }

  function parseCoordinatePoints(text) {
    const points = [];
    String(text || "").split(/\r?\n/).slice(0, 6000).forEach(line => {
      const matches = [...String(line || "").matchAll(/[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?/g)]
        .map(match => Number.parseFloat(match[0]))
        .filter(Number.isFinite);
      if (matches.length < 2) return;
      points.push({ x: matches[matches.length - 2], y: matches[matches.length - 1] });
    });
    return points;
  }

  function renderCoordinateCloudToSvgDataUrl(text) {
    const points = parseCoordinatePoints(text);
    if (points.length < 2) return "";
    const path = [];
    points.forEach((p, idx) => {
      path.push(`${idx === 0 ? "M" : "L"} ${p.x} ${-p.y}`);
    });
    return buildPathSvgDataUrl(path, boundsFromPoints(points));
  }
